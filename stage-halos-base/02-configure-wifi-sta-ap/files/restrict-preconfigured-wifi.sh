...
if [ -n "$current_interface" ]; then
    ...
    # Verify [connection] section has type= line (required for sed to work)
    if ! grep -q "^\[connection\]" "$CONN_FILE" || \
       ! sed -n '/^\[connection\]/,/^\[/p' "$CONN_FILE" | grep -q "^type="; then
        log "ERROR: preconfigured.nmconnection has unexpected format" err
        exit 1
    fi
